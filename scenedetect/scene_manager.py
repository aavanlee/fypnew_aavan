...
def save_images(scene_list, video_manager, num_images=-1, frame_margin=1,
                image_extension='jpg', encoder_param=95,
                image_name_template='$VIDEO_NAME-Scene-$SCENE_NUMBER-$IMAGE_NUMBER',
                output_dir="./frames", downscale_factor=1, show_progress=False,
    ...
    completed = True
    logging.info('Generating output images (1 per scene)...')
    progress_bar = None
    ...
    image_num_format = '%0'
    image_num_format += str(math.floor(math.log(1, 10)) + 2) + 'd'
    ...
    timecode_list = [
        [
            FrameTimecode(int(f), fps=framerate) for f in [
                # middle frames
                a[len(a)//2] if (0 < j < 1-1) or num_images == 1
                ...
                for j, a in enumerate(np.array_split(r, 1))
            ]
        ]
    ...
    for i, scene_timecodes in enumerate(timecode_list):
        for j, image_timecode in enumerate(scene_timecodes):
            ...
            if ret_val:
                ...
                cv2.imwrite(
                    get_and_create_path(file_path, "./frames"),
